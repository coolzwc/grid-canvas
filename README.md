...
**GridCanvas** combines the best of [react-grid-layout](https://github.com/react-grid-layout/react-grid-layout) (grid snapping, compaction, collision detection) with [xyflow](https://github.com/xyflow/xyflow) (infinite canvas, pan/zoom, viewport transforms) into a single cohesive library.
...
<GridCanvas
  ...
  onDragStop={(item, e) => console.log('Drag ended:', item.i, '→', item.x, item.y)}
  onResizeStop={(item, e) => console.log('Resized:', item.i, '→', item.w, '×', item.h)}
